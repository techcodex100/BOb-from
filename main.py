...
def generate_pdf(data: Form144Data):
    # Purpose code-description mapping
    purpose_mapping = {
        "P0103": "Advance receipts against export contracts, which will be covered later by GR/PP/SOFTEX/SDF - other than Nepal & Bhutan",
        # Add more codes here if needed
    }

    # Sync logic
    if data.purpose_code and data.purpose_code in purpose_mapping:
        data.purpose_description = purpose_mapping[data.purpose_code]
    elif data.purpose_description and data.purpose_description in purpose_mapping.values():
        reverse_map = {v: k for k, v in purpose_mapping.items()}
        data.purpose_code = reverse_map[data.purpose_description]
    ...
    # Field positions (x, y, page index)
    positions = {
    ...
    # Fields that require drawing individual digits
    box_fields = [
